    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
      {diseases.map((disease, i) => (
        <motion.div
          ...
          className="bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm flex flex-col group hover:shadow-xl transition-all duration-500"
        >
          ...
          <div className="p-6 flex-1 flex flex-col">
            ...
            <button 
              ...
              className="mt-auto w-full py-3.5 rounded-xl bg-slate-50 text-slate-900 text-[10px] font-black uppercase tracking-widest hover:bg-[#052E16] hover:text-white transition-all active:scale-95"
            >
              বিস্তারিত তথ্য
            </button>
          </div>
  ...
  return (
    ...
    >
      <div className="p-8 md:p-12">
        <button 
          onClick={onBack}
          className="flex items-center gap-3 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-[#052E16] transition-all mb-10"
        >
          <ArrowLeft className="h-4 w-4" /> BACK TO LIBRARY
        </button>

        {/* Hero Section with Header Image */}
        <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl mb-12 border-4 border-slate-50">
          <img src={disease.image} alt={disease.name} className="w-full h-[400px] object-cover" />
        ...
        <div className="grid lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8">
            <div className="flex items-start justify-between mb-16">
               <div>
                  <h1 className="text-6xl font-black text-[#052E16] tracking-tighter mb-2">{disease.name}</h1>
                  <p className="text-xl font-bold text-slate-300 font-bn">{disease.nameBn}</p>
               </div>
               <button className="flex items-center gap-3 px-8 py-4 rounded-2xl bg-[#052E16] text-white text-[11px] font-black uppercase tracking-widest shadow-xl shadow-green-950/20 active:scale-95 transition-all">
            ...
            <div className="space-y-16">
               <section>
                  ...
                  <p className="text-[18px] text-slate-700 leading-[1.8] font-medium font-bn bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                     {disease.description}
                  </p>
               </section>
                  ...
                  <div className="space-y-6">
                     {(disease.symptoms || []).map((s, idx) => (
                       <div key={idx} className="flex gap-6 items-start">
                          <div className="h-3 w-3 rounded-full bg-[#166534] mt-2 flex-shrink-0" />
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="p-10 rounded-[2.5rem] bg-[#2D5A27] text-white shadow-2xl shadow-green-950/20">
               ...
               <div className="space-y-8">
                  {(disease.management || []).map((step, idx) => (
                    <div key={idx} className="flex gap-4">
                       <CheckCircle2 className="h-5 w-5 text-[#EAB308] flex-shrink-0 mt-0.5" />
               ...
               <div className="mt-12 p-6 rounded-2xl bg-white/10 border border-white/10">
