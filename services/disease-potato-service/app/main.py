...
app = FastAPI(title="disease-potato-service", dependencies=[Depends(verify_internal_token)])

try:
    model = YOLO('../../models/potato-disease/best.pt')
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "disease-potato-service"}
