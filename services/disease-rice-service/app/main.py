...
app = FastAPI(title="disease-rice-service", dependencies=[Depends(verify_internal_token)])

try:
    model = YOLO('../../models/rice-disease/best.pt')
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "disease-rice-service"}
